...
void CGL_internal::GConsole::clearBuffer() {
	...
	for (unsigned int y = 0; y < CGL_internal_CONSOLE_HEIGHT; y++) {
		// Get line pointer
		char* lineBegin = &m_consoleBuffer[(CGL_internal_CONSOLE_WIDTH + 1) * y];
		
		// Memset spaces and set newline
		memset(lineBegin, ' ', CGL_internal_CONSOLE_WIDTH);
		lineBegin[CGL_internal_CONSOLE_WIDTH] = '\n';
	}

	// Set terminator char
	m_consoleBuffer[(CGL_internal_CONSOLE_WIDTH + 1) * CGL_internal_CONSOLE_HEIGHT] = '\0';
}
